...
function selectOptions() {
	selectTg.forEach((selectTag, id) => {
		for (let countryCode in countries) {
			const selected =
				(id === 0 && countryCode === "en-US") ||
				(id === 1 && countryCode === "de-DE")
					? "selected"
					: "";
			const option = `<optiom ${selected} value =${countryCode}>${countries[countryCode]}</optiom>`;
...
function handleIconClick(target) {
	...
	if (target.classList.contains("fa-copy")) {
		...
		if (target.id === "from") {
			...
			utterance = new SpeechSynthesisUtterance(tpText.value);
